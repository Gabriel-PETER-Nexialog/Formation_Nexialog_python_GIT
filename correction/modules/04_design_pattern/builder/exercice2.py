...
class OffreHabitationBuilder:
    ...
    def avec_proprietaire(self, nom: str):
        self._offre.proprietaire = nom
        return self

    def avec_adresse(self, adresse: str):
        self._offre.adresse = adresse
        return self

    def avec_surface(self, surface_m2: float, nb_pieces: int):
        self._offre.surface_m2 = surface_m2
        self._offre.nb_pieces = nb_pieces
        return self

    def avec_type_bien(self, type_bien: str):
        self._offre.type_bien = type_bien
        return self

    def avec_formule(self, formule: str):
        self._offre.formule = formule
        return self

    def avec_garantie(self, garantie: str):
        self._offre.garanties.append(garantie)
        return self

    def avec_franchise(self, franchise: float):
        self._offre.franchise = franchise
        return self

    def build(self) -> OffreHabitation:
        if not self._offre.adresse:
            raise ValueError("L'adresse du bien est obligatoire.")
        return self._offre
